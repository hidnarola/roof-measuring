  ...
  methods: {
    initMap() {
      ...
      this.map.eachLayer((layer) => {
        if (layer._latlngs) {
          _finalObject.shape && _finalObject.shape.map((shapes, i) => {
              shapes.path.map((pathPoint, j) => {
                layer._latlngs && layer._latlngs[0].map((_latlng) => {
                    if (pathPoint[0].lat == _latlng.lat && pathPoint[0].lng == _latlng.lng ) {
                      _finalObject.shape[i].area = parseFloat( Math.round( L.GeometryUtil.geodesicArea(layer._latlngs[0])));
      ...
      _finalObject && _finalObject.shape.map((shape, i) => {
          shape && shape.path.map((pth) => {
              // setting out the measurement and handleing color details
              ...
              if ( _finalObject.measurement && _finalObject.measurement[pth[0].label] ) {
    ...
    async handlePdf() {
      ...
      for (var i = 0; i < this.polyData.length; i++) {
        ...
        for (var j = 0; j < this.polyData[i].length; j++) {
          point = this.map.latLngToLayerPoint([this.polyData[i][j][0], this.polyData[i][j][1], ]);
